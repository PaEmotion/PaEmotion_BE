...
## 테스트용
## ===== sliding window 방식으로 데이터셋 구성 =====
def make_features_with_info_test(df, window=8):
    X, y, info = [], [], []
    for user in df['userId'].unique():
        user_df = df[df['userId'] == user].sort_values('week')
        spend = user_df['spendCost'].values
        ...
        print(f"user {user} spend length: {len(spend)}, weeks: {user_df['week'].values}")
...
def full_preprocess_test(df, window=8, cat_window=3):
    weekly_spending = preprocess_weekly_spending(df)
    cat_feat = add_category_repeat_features(df, window=cat_window)
    weekly = pd.merge(weekly_spending, cat_feat, on=['userId', 'week'], how='left').fillna(0)
    weekly = add_main_routine_avg(weekly)
    X, y, info = make_features_with_info_test(weekly, window=window)
    for user in weekly['userId'].unique():
        weeks = weekly[weekly['userId'] == user]['week'].values
        print(user, weeks)
    return X, y, info